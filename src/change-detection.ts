...
function byName(teams: TeamRecord[]): Map<string, TeamRecord> {
  return new Map(teams.map((team) => [team.normalizedName, team]));
}
...
export function applyResults(
  ...
  const tournaments = configs.map((config): TournamentState => {
    ...
    const pending = new Map(old.pendingRemovals.map((item) => [item.team.normalizedName, item]));

    if (old.lastSuccessfulCheck) {
      for (const team of result.teams) {
        const prior = oldTeams.get(team.normalizedName);
        pending.delete(team.normalizedName);
        if (!prior) {
      ...
      for (const team of old.teams) {
        if (newTeams.has(team.normalizedName)) continue;
        const existing = pending.get(team.normalizedName);
        if (existing && existing.observations >= 1) {
          ...
          pending.set(team.normalizedName, {
            team,
